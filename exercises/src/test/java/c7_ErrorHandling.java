...
//                .onErrorComplete(); //normalement c'est ça la réponse
                .onErrorResume(e -> Flux.empty()) // mais ça marche aussi
        //todo: change this line only
        ;
    ...
    public void have_a_backup() {
        ...
        Flux<String> messages = messageNode()
                .onErrorResume(RuntimeException.class,e ->  backupMessageNode());
    ...
    public void error_reporter() {
        //todo: feel free to change code as you need
        Flux<String> messages = messageNode()
                .publishOn(Schedulers.boundedElastic()) // bizarre....
                .doOnError(e -> errorReportService(e).subscribe());
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap(file -> file.onErrorResume(e -> Mono.empty())) //start from here
                .map(Function.identity()); //start from here
